...
// V4.0-003: 材料编辑器
#[tauri::command] pub async fn material_templates(cat: Option<String>) -> Result<serde_json::Value, String> {
    match cat { Some(c) if !c.is_empty() && c != "all" => py_val(format!("from caelab.product.tools import list_material_templates; import json; print(json.dumps(list_material_templates('{}'), ensure_ascii=False))", c)).await, _ => py_val("from caelab.product.tools import list_material_templates; import json; print(json.dumps(list_material_templates(), ensure_ascii=False))".into()).await }
}
#[tauri::command] pub async fn material_get(id: String) -> Result<serde_json::Value, String> { py_val(format!("from caelab.product.tools import get_material_template; import json; print(json.dumps(get_material_template('{}'), ensure_ascii=False))", id)).await }
#[tauri::command] pub async fn material_categories() -> Result<serde_json::Value, String> { py_val("from caelab.product.tools import material_categories; import json; print(json.dumps(material_categories(), ensure_ascii=False))".into()).await }
#[tauri::command] pub async fn material_compare(ids: Vec<String>) -> Result<serde_json::Value, String> { let js = serde_json::to_string(&ids).unwrap(); py_val(format!("from caelab.product.tools import compare_materials; import json; print(json.dumps(compare_materials({}), ensure_ascii=False))", js)).await }
// V4.1-003: 置信度
#[tauri::command] pub async fn confidence_compute(data: serde_json::Value) -> Result<serde_json::Value, String> { py_val(format!("from caelab.product.tools import compute_confidence; import json; print(json.dumps(compute_confidence({}), ensure_ascii=False))", data)).await }
#[tauri::command] pub async fn confidence_annotate(field: String, values: Vec<f64>) -> Result<serde_json::Value, String> { let vs = serde_json::to_string(&values).unwrap(); py_val(format!("from caelab.product.tools import annotate_result_field; import json; print(json.dumps(annotate_result_field('{}',{}), ensure_ascii=False))", field, vs)).await }
